...
int run(int flag, int argc, char* argv[]) {
    ...
    ape::monitor::ConfigManager::GetInstance()->Init(configfile);
	ape::monitor::WatchDogThread::GetInstance()->Start();
    ape::monitor::CMongoThreadGroup::GetInstance()->Start(parser.GetParameter("dbthread", 3));
    ape::monitor::CMongoThreadGroup::GetInstance()->OnConnect(parser.GetParameter("db/mongo", "mongodb://127.0.0.1"));
    ape::monitor::CCollectThread::GetInstance()->Start();
    
    std::vector<std::string> addrs = parser.GetParameters("server/addr");
    ape::net::NetHandleFactory<ape::monitor::CBusinessHandle> factory;
